...
const _AMOUNT = 5
const _STOP = _AMOUNT - 1
const ITEM_NAME = 'The Lightning #'
const ITEM_DESCRIPTION = 'BUY ME NOW!!!!'
  ...
  for (let index = 0; index < unsortedFiles.length; ++index) {
    ...
    if (compareFileNames(element1, element2)) {
      await imageToBase64("./racecars/" + element2) // path to the image
        ...
        .catch((error) => {
          console.log(
            chalk.yellow.bold(`Error in imageToBase64 ${error} .............`)
          );
        });
    }
  }
...
async function uploadMyNFT(metadata, assetNumber, base64ImageData) {
  let newAssetName = `BETA${assetNumber}`
  const body = {
    AssetName: newAssetName,
    previewImageNft: {
      mimetype: "image/png",
      displayName: ITEM_NAME + assetNumber,
      ...
      metadataPlaceHolder: [
        ...
        {
          name: "Glass",
          value: metadata.Glass,
        },
        {
          name: "Sticker",
          value: metadata.Sticker,
        },
      ],
    },
  };
  ...
  await axios(config)
    .then(function (response) {
      console.log(
        chalk.blueBright.bgBlackBright.bold(
          `🚀 Success ${assetNumber} has been uploaded`
        )
      );
    })
...
async function* asyncGenerator() {
  ...
  while (i < _AMOUNT) {
    ...
    const assetNumber = filesToUpload[i].fileReference.padStart(3, "0"); // (5, "0")
    ...
    if (num === _STOP) {
      console.log(
        chalk.magentaBright.bold(
          "Finished generating images, now Axios will upload the shit. lmao..."
        )
      );
      process.exitCode = 0;
    }
